...
- 双亲和子组件之间的通信或者层级较深的祖孙组件。
- 共享功能（全局设置、权限控制等）
- 插件和库的实现，比如Vue Router 使用 `provide` 来提供路由信息，Vuex 使用 `provide` 来注入 `store`，这些都避免了每个组件都需要显式传递数据的问题。
- 动态的注入数据
...
注意：注入和依赖提供的属性是非响应式的。但是可以通过 Vue 的响应式 API（如 `reactive` 或 `ref`）来实现响应式数据的传递

### ref / $refs

应用场景：双亲和子组件上

ref在子组件上，它的引用就指向了子组件的实例。

### $parent / $children

`$parent`可以让组件访问上级组件的实例（一个对象)

`$children`可以让组件访问子组件的实例，但是，`$children`并不能保证顺序（一个数组），并且访问的数据也不是响应式的

### $attrs / $listeners

应用场景：组件之间的跨代通信

`$attrs`：继承所有的双亲组件属性（除了prop传递的属性、class 和 style ），一般用在子组件的子元素上
